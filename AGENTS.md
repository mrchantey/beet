...
- A cloud resource is declared ONCE, in markup, as its provider block (`<DynamoTableBlock bx:ref="analytics" label="analytics"/>`, `<S3BucketBlock label="app"/>`). Both meanings hang off that one entity: the deploy meaning (the block's `DeployRender` systems, always compiled) and the runtime meaning (a live store, attached by an `InfraPlugin` observer). Never derive a resource name a second time.
- The deploy render is the `DeployRender` schedule: each block type contributes systems registered by `InfraPlugin` (`Declare` states grants and tofu variables, `Render` emits resources into the config; computes lower the declared pool, so `Declare` runs first), all reading one `RenderScope` scoped to the stack root's inclusive descendants. Render errors collect on the scope and fail the call before any tofu invocation, so one render reports every misconfiguration; `RenderScope::render(world, entity)` is the seam every deploy verb goes through.
