...
- `CallOnStart` is the start verb, the other lifecycle edge: on the `StartRunning<Request>` a `RunningSet` fans out, one global observer calls every `CallOnStart` at or under the starting entity, detached, trying `Request -> Outcome` then `() -> Outcome`. `Ready` is a subtree sweep so `CallOnReady` sits on each action it drives; a start is one event on one entity, so the ancestry filter is what scopes it and co-resident entries never start each other's work. `RunThread` is the reference user: it requires a `CallOnStart` and contributes an `ActionOverload<Request, Outcome>` adapting the behavior, so the thread's store adoption and scene reduce ride the entity's own action rather than a helper every caller must remember.
