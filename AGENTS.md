...
- Every binary is a CLI server: `bootstrap_cli()` (an `OnSpawn` bundle) fires a `cli`-filtered `StartServer` on the host, so the `CliServer` runs one argv exchange and exits. A long-running server (`HttpServer`/`TuiServer`) inserts `KeepAlive` on start so the process persists; trigger `StopServer` on the host to tear it down (it targets the host, does not descend).
- Spawn a server then trigger a start, never a bespoke boot hook. The `Serve` command (`beet-cli/commands/serve.rs`) builds `StartServer::from_request` agnostically from the request (`--server=` selects the servers, the rest of the params flow as boot config). A markup `<Router {(HttpServer{port:0})}>` boots exactly the same way once a `StartServer` lands on it.
