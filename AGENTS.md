...
- **Durable data born at RUNTIME gets its own bucket.** The app bucket is deploy-mirrored content: the sync owns that tree (and prunes what it did not put there) and the runtime holds a read-only grant on it. `<S3BucketBlock label="runtime-ops" runtime_write=true object_versioning=true deploy_versioned=false/>` is the inverse, append-mostly, never touched by a deploy sync, and the only bucket the running process may write, so least privilege stays crisp on both sides. Analytics owns the `analytics/` prefix in it; other ops-durable data (table exports, generated reports) joins under its own prefix, and user-owned data would get its own bucket and its own grants, never this one.
- **Expiry is the LAST step of a retention order, never a policy on its own.** `<DynamoTableBlock ttl="ttl"/>` enables only the attribute DynamoDB reads an expiry from (an in-place update to a live table: none of the fields a replacement would move is touched), and a row without that attribute is never deleted, so the WRITER decides what is expirable. For analytics the order is pinned in code (`AnalyticsRollupRun`): archive the day cold to the runtime-ops store as `analytics/raw/<date>.ndjson.gz`, roll it up into `AnalyticsRollup` rows that carry no `ttl` and outlive everything, confirm both by reading them back, and only then stamp that day's raws. `AnalyticsRetention` (a component resolved by ancestry, so one declaration covers both the recorder and the job) rejects a window shorter than the job's cadence, and a backfilled row already past its window takes a `GRACE` floor rather than an expiry in the past. A run covers every COMPLETE day not already both archived and aggregated, so a week the schedule missed is picked up rather than expiring unarchived; `--full` re-sweeps everything, which is safe because aggregate ids and archive object names are pure functions of the day they cover.
