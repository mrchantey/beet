...
- `CallOnLoad` is the one load verb: on the entity's `LoadTemplate` it calls the entity's action with the load's request and streams/exits. It tries `Request -> Response`, then `() -> Outcome` (`Pass` exits zero, `Fail` nonzero), then `() -> ()`, so a behavior scene is just `<Sequence {CallOnLoad}>`. Loading never implies running: the request is the load *context* (`LoadRequest`, resolved from the nearest ancestor), provided by whoever builds the tree, so a command building a document to render or inspect simply provides none and nothing fires.
