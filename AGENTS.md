...
- A declaration carries only its **label**. `Stack` is the single composition turning it into `<app>--<stage>--<label>`: a markup-authorable identity component (`app_name`, `stage`, `region`), resolved from the nearest ancestor `<Stack>` else the process default (`PackageConfig::app_name` + `BootstrapConfig::stage` + `AWS_REGION`), which is the only in-world read of that variable. One-launch mechanics (the state backend, the work dir, the deploy id every artifact is keyed by) live beside it on the `Deployment` resource.
