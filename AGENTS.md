...
- Servers are entity events, no orchestrator (`server/server_events.rs`). Each server component (`CliServer`, `HttpServer`, `TuiServer`) registers entity-scoped `StartServer` / `StopServer` observers in its `on_add` hook; an observer boots/stops only if the event passes its name (`ev.passes("http")`). An empty/default `GlobFilter` matches every present server. `params` (eg `--port`, `--host`) flow from whoever triggered the start, never read from `CliArgs::parse_env` inside the server.
- Every binary is a CLI server: the `beet` binary discovers an entry whose router declares the transports plus a `ServeOnLoad` verb. On `LoadTemplate`, `ServeOnLoad` fires a `StartServer` event filtered by `--server` (empty matches every present server), so the default invocation boots the `CliServer`, which runs one argv exchange and exits. A long-running server (`HttpServer`/`TuiServer`) inserts `KeepAlive` on start so the process persists; trigger `StopServer` on the host to tear it down (it targets the host, does not descend).
- Spawn a server then trigger a start, never a bespoke boot hook. The `ServeOnLoad` verb builds `StartServer::from_request` agnostically from the entry's CLI args (`--server=` selects the servers, the rest of the params flow as boot config). A markup `<Router {(HttpServer{port:0})}>` boots exactly the same way once a `StartServer` lands on it.
- `ServerPlugin` installs the compile-time-selected `HttpServer` backend (mini / hyper / lambda) via `set_http_server`, or nothing if no server feature; a no_std adapter installs its own. `HttpServer`'s start observer just invokes the installed `HttpServerFn`. Static generation is its own one-shot command (`beet export-static <site>`), not a serve mode.
