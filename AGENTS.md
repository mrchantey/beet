...
- A RUNTIME consumer names the declaration through a relation, never a name: `<Router {(AnalyticsConfig, StoreRef($analytics))}>`. A consumer with nothing to point at is a loud error naming the relation it is missing, not a silent local fallback. One relation per store the consumer distinguishes, purpose-named (`StoreRef` is the primary; the rollup job adds `RollupStoreRef` for its aggregate table and `ArchiveStoreRef` for the store it archives into), since a relationship is one-per-type and a consumer touching three stores must say which is which. `StoreRef::resolve` is how any of them reads the erased store off the declaration, backing off first because a declaration's runtime half lands through the command queue. A DEPLOY-time reference from one block to another rides the same relation model (`VpcRef`, `DatabaseRef`, `InvokeTarget`, each targeting the declaration entity via `bx:ref`): the target block owns the ident composition, the consumer's render system reads the target's block component through the relation and asks it to compose, and a missing, dangling or out-of-stack target is a collected render error naming the relation, before `tofu plan` ever runs. `SecretRef` stays a name composition (`/app/stage/label`), not a relation: a secret is created by `EnsureSecret` at deploy time, so there is no declaration entity to target.
- A recurring job is a declaration like any other: `<ScheduledJobBlock label="rollup-daily" {InvokeTarget($rollup_fn)} schedule="cron(0 3 * * ? *)" path="rollup"/>` renders an EventBridge schedule plus an invoke role scoped to the one function its `InvokeTarget` relation names, and declares no grants of its own (what the job may touch is the target lambda's own declared grants, lowered as usual). The expression is validated at render, so a typo fails the deploy rather than the job. What an invoke delivers is a `ScheduledInvoke` (`beet_net`), the ONE payload type the block serializes and the lambda adapter deserializes, tagged and versioned because a schedule keeps invoking with the payload the last deploy rendered: `lambda_http` hands the non-http event over as a pass-through, the adapter dispatches it through `exchange_child` exactly like an http request, and its status resolves the INVOCATION rather than a response body, since a job whose dispatch failed must fail the invoke or it reports green in every metric the schedule publishes.
- **A job lambda publishes nothing.** `<LambdaJobBlock label="rollup" exec_route="jobs" features=".."/>` is the invoke-only counterpart of `<LambdaSiteBlock/>`: `LambdaBlock`'s `http=false` renders the function, its role, its log group and its lowered runtime policy, and NOT the function url, gateway, route, stage or anonymous invoke permission. The gateway authorizes nothing (`authorization_type` is `NONE`), so a sweep that rewrites every row in a table would otherwise sit behind a url anyone can hold open; a hostname on such a block fails the deploy rather than publishing a record at nothing. It boots like any other lambda: the runtime offers no argv, so `remote_bootstrap` bakes into the zip's `bootstrap` and `exec_route` names the verb whose `Router` the invoke's `ScheduledInvoke` path dispatches into. The job therefore runs a route of the same entry document the site serves from, in its own url space and its own process, so co-residence in one file costs the served site no reachable surface. `LambdaBlock::timeout_secs` is 180 by default (what a served function wants) and the job template takes the service maximum, since a sweep over a store is not a request.
- **A bucket is one of three profiles, and the LABEL names the profile.** `app` is deploy-mirrored content: the sync owns that tree (and prunes what it did not put there) and the runtime holds a read-only grant on it. `archive` is its inverse, `<S3BucketBlock label="archive" runtime_write=true object_versioning=true deploy_versioned=false/>`: append-only cold data no deploy sync ever touches and the only bucket the running process may write, so least privilege stays crisp on both sides. It holds backup dumps AND sole-copy archives (the analytics raws under `analytics/raw/`, whose table rows expire once archived), which is why it is not called `backups`: a bucket named for backups invites treating a sole copy as expendable. `user-data` is reserved for beet-owned runtime-written primary data of any origin (chat blobs, ingested mail bodies) and is declared when its first consumer lands, never unconsumed. The `assets` buckets (public, stage-shared) are a public variant of the `app` profile.
- **Sharing a bucket means sharing a WRITE GRANT.** A grant is whole-bucket (`Block::grants` names the bucket, never a prefix), so prefix ownership inside one is writer convention and nothing more: co-locate two datasets only when their writers already trust each other, and give data with its own trust boundary its own bucket and its own grants. What a prefix CAN say is retention: `expire_prefixes={[{prefix:"postgres/", expire_days:180}]}` renders one filtered lifecycle rule beside the unfiltered `expire_days` / `expire_noncurrent_days` ones (all on the single configuration a bucket may have), so an `archive` expires a nightly database dump while the sole copy beside it is kept forever. The trailing slash is load-bearing (`postgres` also matches `postgres-old/`), a rule id derives from its prefix rather than from declaration order so a reorder never diffs a rendered configuration, and a zero-day or id-colliding rule fails at render rather than at apply.
- **Expiry is the LAST step of a retention order, never a policy on its own.** `<DynamoTableBlock ttl="ttl"/>` enables only the attribute DynamoDB reads an expiry from (an in-place update to a live table: none of the fields a replacement would move is touched), and a row without that attribute is never deleted, so the WRITER decides what is expirable. For analytics the order is pinned in code (`AnalyticsRollupRun`): archive the day cold to the `archive` store as `analytics/raw/<date>.ndjson.gz`, roll it up into `AnalyticsRollup` rows that carry no `ttl` and outlive everything, confirm both by reading them back, and only then stamp that day's raws. `AnalyticsRetention` (a component resolved by ancestry, so one declaration covers both the recorder and the job) rejects a window shorter than the job's cadence, and a backfilled row already past its window takes a `GRACE` floor rather than an expiry in the past. A run covers every COMPLETE day not already both archived and aggregated, so a week the schedule missed is picked up rather than expiring unarchived; `--full` re-sweeps everything, which is safe because aggregate ids and archive object names are pure functions of the day they cover.
...
- Permissions are declared by the resource (`Block::grants` -> `AccessGrant`, contributed to the `RenderScope` by the generic declare system) and **lowered** by the compute block, which for the AWS computes is the shared `IamPolicy`: it seeds the statements the compute needs on its own account (an artifacts bucket, a log group), lowers the stack's grants into read/write bucket and per-table statements, and renders them in call order. A compute whose lowering yields nothing emits no policy resource at all, never a managed `FullAccess` one. (Per-compute needs ride knobs, never forks: the ssm parameter kind lowers in the shared core, and `write_action` adds `s3:AbortMultipartUpload` for the mail box's multiparting blob store without widening any other compute's write set.) A resource block never writes an ARN; a compute block never names a sibling resource. An `AccessGrant` is `{kind, name, permissions}`: `kind` is a plain string constant the declaring block owns (`S3BucketBlock::ACCESS_KIND`), so a new provider mints `"r2_bucket"` without touching shared code, and the ARN region comes from the compute's own resolved stack rather than the grant. Lowering is **loud on unknown**: a kind the compute cannot lower fails the deploy naming both the kind and the compute, and there are no `_ =>` catch-alls in grant handling (a silently dropped grant is a box that serves until the first request touching that resource).
