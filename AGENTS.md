...
- `CallOnStart` is the start verb, the other lifecycle edge, and the same shape as the load verb: `Ready` sweeps a loaded subtree, and a start sweeps the started one. `StartRunning` rides a `ScopedTrigger`, which reads `StartDescendants` off its target: with it the event sweeps the subtree (deepest first, root last), without it it fires on the target alone. `RunningSet` `#[require]`s `StartDescendants`, so every run start sweeps and `CallOnStart` observes its own entity, calling its action with the start request, detached, trying `Request -> Outcome` then `() -> Outcome`; a bare `ContinueRun` start stays single-target, so behavior-tree hot paths never pay for a sweep. The sweep never leaves its root's subtree, so co-resident entries never start each other's work. `RunThread` is the reference user: it requires a `CallOnStart` and contributes an `ActionOverload<Request, Outcome>` adapting the behavior, so the thread's store adoption and scene reduce ride the entity's own action rather than a helper every caller must remember.
