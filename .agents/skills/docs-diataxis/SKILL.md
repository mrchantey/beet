...
If the task is auditing, restructuring, or improving existing docs rather than writing one new piece, load `docs-improving`.

## Working principle

Do not try to design the whole documentation structure up front. Good structure emerges from incremental improvement: pick one piece, make it serve its single mode better, publish, repeat. See `docs-improving` for the full workflow.
