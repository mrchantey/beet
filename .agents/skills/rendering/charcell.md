...
- **Raster `<img>`** decodes to PNG (`kitty.rs` `to_png_bytes`: PNG passes through, JPEG/GIF/WebP decode via the `image` crate, `tui`-gated) and a site-rooted `src` (`/assets/…`) is **fetched over HTTP** from our own server (`RenderServerOrigin: Url`, set from the running `HttpServer`'s loopback addr by `set_render_server_origin`, all `tui+net`-gated in `beet_ui`) exactly like the browser — the server maps it to its blob store, so the render host needs no filesystem (scales to Lambda/Fargate; a bare path is a last-resort local read for tests). **An HTTP server must be listening**: `--server=tui` alone has no origin → the `[image]: alt` marker shows; run `--server=http,tui` (or any http mode; the ssh/deploy case always serves http). `tui_server` pulls `beet_ui/net` for the fetch. Sizing honours the box (`cell_size_constrained`). Verify a render by forcing kitty (`TERM=xterm-kitty` under a PTY) and grepping the raw output for `\x1b_Ga=t` (transmit) / `\x1b_Ga=p` (place).
