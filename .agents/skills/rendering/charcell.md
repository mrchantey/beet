...
- **Raster `<img>`** decodes to PNG (`kitty.rs` `to_png_bytes`: PNG passes through, JPEG/GIF/WebP decode via the `image` crate, `tui`-gated) and a site-rooted `src` (`/assets/…`) is **fetched over HTTP** from our own server (`RenderServerOrigin`, set from the `HttpServer`'s loopback addr by `RouterPlugin`) exactly like the browser — the server maps it to its blob store, so the render host needs no filesystem (scales to Lambda/Fargate; a literal fs path is a last-resort local read for tests/offline). Sizing honours the box (`cell_size_constrained`). `tui_server` pulls `beet_ui/net` for the fetch.
- **Hover state layer in every scheme.** `HoverSurface` is bound at `:root` (light *and* dark, `scheme_independent_declarations`), not light-only. A light-only binding left it *unset* in dark, so a hovered interactive's `background` redirected to nothing → `None`; on a full-width row (a sidebar link with a base fill) the transition then snaps the fill away mid-hover, the row's cells leak back to the rail, and the painted-cell hit-test flickers the hover on/off. Glyph cells are immune (the text keeps painting). The tell is the *empty* cells of a full-width interactive flickering in one scheme only.
