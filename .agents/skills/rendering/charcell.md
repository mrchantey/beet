...
- **Raster `<img>`** decodes to PNG (`kitty.rs` `to_png_bytes`: PNG passes through, JPEG/GIF/WebP decode via the `image` crate, `tui`-gated) and a site-rooted `src` (`/assets/…`) resolves against `RenderAssetRoot` (mirrored from `SiteRoot` by `RouterPlugin`), so the terminal reads the same on-disk file the web fetches over HTTP. Sizing honours the box (`cell_size_constrained`).
