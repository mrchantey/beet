...
Beet has downstream repos (separate git repos building on beet via a path dependency). Each inherits beet's conventions rather than fracturing into its own: a verbatim copy of beet's `AGENTS.md` lives inside a marker block at the bottom of the downstream `AGENTS.md`, and shared config files are copied as-is. This skill refreshes all of that.
...
Afterwards spot-check one downstream `AGENTS.md`: header intact, exactly one block, beet's current text inside it.
