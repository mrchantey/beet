...
The analytics segment namespace does not grow forever. A `<ScheduledJobBlock label="rollup-daily"/>` invokes the `rollup` lambda at 03:00 UTC, which boots the entry's `jobs` verb out of the app bucket and dispatches `rollup` into its router. Each run merges every complete day's segments with its existing `analytics/raw/<date>.ndjson.gz` archive in `beet-site--<stage>--archive`, writes that day's deterministic `AnalyticsRollup` rows into the JSON-over-blob `beet-site--<stage>--analytics-rollup` bucket, reads both outputs back, and only then deletes the source segments from `beet-site--<stage>--analytics`. The daily archive is the compact primary raw history; rollups are kept and can be re-derived from it.
...
- ON PROD, once the schedule has been firing, `0 days` is the normal daytime result because 03:00 UTC compacted yesterday. Verify coverage independently: every completed date formerly represented by segments has `analytics/raw/<date>.ndjson.gz` in the archive bucket and a site-scope aggregate object in the rollup bucket. Any leftover complete-day segment should be consumed by the next run.
