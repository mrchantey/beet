...
The site records analytics for every transport: a server `Request` event per routed request, a `PageView` (with dwell) per web/terminal page visit, and `Click` / `Scroll` / `Error` events from the web client beacon. Events buffer into immutable gzip NDJSON segments. Verify the visits from checks b-d landed in the store and that prior segments and rollups remain visible. Query with the `beet analytics` subcommand, mounted in `site/main.bsx` beside the `analytics` declaration it reads (built with `aws_sdk` so `--service-access=remote` reaches the live S3 bucket):

- local: `cargo run -p beet-cli -- --main=site analytics summary` (the declared store, `target/stores/beet-site--dev--analytics`)
- dev/prod: `cargo run -p beet-cli --features infra -- --main=site --stage=<stage> --service-access=remote analytics summary`, or name a store outright with `--store s3://beet-site--<stage>--analytics`
...
- `beet analytics summary` now reports over BOTH stores: aggregates for the days they cover, raws for the recent window they do not. `--raw-only` skips the aggregates, `--rollup <uri>` overrides the aggregate store (it defaults to the `RollupStoreRef` declaration, else the events store).
