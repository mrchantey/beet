...
Do NOT read a single `curl: (22) The requested URL returned error: 500` in that step's narration as a failure. The script logs the stderr of EVERY attempt, so one failed probe followed by a pass is a converge loop doing its job, and on a fresh box the first request routinely is that failure: the box's very first `/` is served while the instance is still at zero burst capacity, and it 500s on an AWS SDK `dispatch failure` reaching the entry store, the same clamp described under Topology. The tell that it was transient rather than a crash loop is the box log — request `#1 -> 500`, `#2 -> 200 OK` — plus `NRestarts=0` afterwards. A `500` that repeats across attempts fails the gate on its own and the deploy never reaches this line.
...
The analytics table does not grow forever. A `<ScheduledJobBlock label="rollup-daily"/>` invokes the `rollup` lambda at 03:00 UTC, which boots the entry's `jobs` verb out of the app bucket and dispatches `rollup` into its router. Each run, in this order and no other: archives every complete day not yet covered as `analytics/raw/<date>.ndjson.gz` in `beet-site--<stage>--archive`, writes that day's `AnalyticsRollup` rows into `beet-site--<stage>--analytics-rollup`, reads both back, and only then stamps the day's raw rows with the `ttl` DynamoDB expires them by (30 days for requests, 90 for the client-reported streams). The aggregates and the archive are forever; the raws are not.
...
- ON DEV, `0 days` IS THE ONLY POSSIBLE HONEST ANSWER, so an invoke on its own proves nothing but that the function boots. `AnalyticsRollupRun::dates` never covers today, and a dev stack stood up and torn down inside one UTC day holds today and nothing else. To verify the pipeline rather than the plumbing, seed a complete day into `beet-site--dev--analytics` first: a handful of `Request` rows timestamped yesterday and carrying NO `ttl` (the recorder stamps its own, and the run leaves a stamped row alone, so an unstamped row is what makes step 3 observable). Then invoke, and assert all three steps landed — the archive object exists and its ndjson still shows `"ttl"` absent (it was written BEFORE the stamp, which is the ordering), the aggregate rows are there, and the seeded raws now carry a `ttl` of the event's day plus the 30-day request window. Re-invoking must then report `0 days`: the day is covered. A live run of exactly this reported `1 days from 602 scanned rows / archived: 1 / aggregates: 4 / expiring: 3 raw rows stamped`.
- ON PROD, ONCE THE SCHEDULE HAS BEEN FIRING, `0 days` IS ALSO THE HONEST ANSWER and `expiring: 0 raw rows stamped` is the steady state. The 03:00 UTC run covers yesterday, so by the time a daytime deploy is verified every complete day is already archived and aggregated; and the LIVE RECORDER stamps each row's `ttl` at write time (`AnalyticsStore::record` -> `AnalyticsEvent::with_retention`), while step 3 only stamps rows where `ttl.is_none()`. So a prod archive object legitimately shows `"ttl"` PRESENT on every row — the ndjson-has-no-ttl assertion above belongs to the dev seeding recipe alone, where the seeded rows deliberately carry none. Read the order off the last real scheduled run instead: its CloudWatch report names the day, the objects archived and the aggregate rows, and `archived: N > 0` with `expiring: 0` is the exact INVERSE of the failure shape (archived nothing, stamped rows). Verify coverage independently rather than trusting the `0`: every complete day in the raw table must have both an archive object and aggregate rows, and no aggregate row may carry a `ttl`.
- A long-dwell `PageView` is upserted with a fresh timestamp as its dwell grows, so a row archived under day N can move to day N+1 and appear in day N's archive AND day N+1's aggregate. That is not lost history: archive objects and aggregate ids are pure functions of the day, so the overlap is idempotent. When diffing an archive against the live table, look up an "archive-only" id before calling it a loss — it is usually alive under a later day.
...
EYEBALL the plan for three things specifically: that `aws_s3_bucket.beet_site__prod__app` and `aws_dynamodb_table.beet_site__prod__analytics` appear only as *refreshes* or in-place updates, never destroys (they carry the site content and the whole analytics history, and enabling `ttl` must be an update); that the new resources (the `analytics-rollup` table, the `archive` bucket, the `rollup` lambda and the `rollup-daily` schedule) are additions; and that the DNS changes are the ones you expect.

A BASELINE REDEPLOY OF UNCHANGED MAIN PLANS EXACTLY ONE CHANGE. Prod already holds every resource, so a code-only deploy reads `Plan: 0 to add, 1 to change, 0 to destroy`: the rollup lambda's `s3_key`, `source_code_hash` and its two `BEET_DEPLOY_*` variables. Everything else, the app bucket, both analytics tables, the archive bucket, the instance and all three Cloudflare records, appears as a `Refreshing state...` line and nothing more. The box itself is NOT in the plan at all: the binary reaches it through the release pointer, so `uptime` on the box spans previous deploys and an instance uptime that resets on a code-only deploy means `user_data` moved.
