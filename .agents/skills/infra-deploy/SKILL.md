...
The site records analytics for every transport: a server `Request` event per routed request, a `PageView` (with dwell) per web/terminal page visit, and `Click` / `Scroll` / `Error` events from the web client beacon. Events buffer into immutable gzip NDJSON segments. Verify the visits from checks b-d landed in the store and that prior segments and rollups remain visible. Query with the `beet analytics` subcommand (built with `aws_sdk` so `--remote` reads the live S3 buckets, not a local fallback):
