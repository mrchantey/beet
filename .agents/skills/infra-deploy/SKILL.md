---
name: infra-deploy
description: The release process for the beet website. Validate the same site across local, dev and prod in sequence with an identical verification pass, tearing dev down after. Use for any site deploy, teardown or deploy verification. Invariants live in crates/beet_infra/README.md.
---
...
- **http**: Caddy on the box terminates TLS for every hostname with an automatic Let's Encrypt cert and reverse-proxies to the beet binary on its app port. Cloudflare fronts the site hostnames PROXIED (an `A` record at the static IP) and runs Full-strict, so the edge verifies Caddy's cert. The certificate store OUTLIVES the box: a `caddy-backup.timer` on the box saves `/var/lib/caddy` to `s3://beet-site--<stage>--repo/caddy/` every 15 minutes and cloud-init restores it before Caddy starts, so a rebuilt box serves on the certificate the last one held (good for 90 days, renewed by Caddy a month early) and only the FIRST box of a stage ever issues at boot. The box's IAM key may write that prefix alone, never a release.
...
REGRESSION GUARD (fixed): a boot-time Let's Encrypt failure left the box serving TLS the edge rejects, with nothing in the deploy that notices (`<LightsailRelease/>` probes loopback). Twice live: once the chain download failed after a successful validation, and once (2026-09-11) a rebuilt prod box came up during a Let's Encrypt secondary-validation outage with its first attempts already lost to the firewall race, so `beet.org` and `www.beet.org` served `525` for two hours (distinct from `521`: something IS listening on 443, its cert is just not one Full-strict accepts) while the certificate the previous box had held was valid the whole time. A rebuild now restores the saved certificate store before Caddy starts, and Caddy starts from the release step rather than racing the firewall. Covered by `rebuild_restores_the_certificate_store` and `release_starts_the_tls_terminator` in `crates/beet_infra/src/blocks/lightsail.rs`. On a `525` check `sudo find /var/lib/caddy -name '*.crt'` for the MISSING hostname and `sudo journalctl -u caddy`; if Let's Encrypt itself is failing (`During secondary validation`, `serverInternal`) check their status page and community forum before restarting Caddy, since every restart burns validation attempts against a limit of 5 per hostname per hour.
...
- `/blog` -> 200, and the body carries a `/blog/<slug>` link per post. The listing is generated by `<RouteIndex reverse="true"/>` in `site/routes/blog/index.bsx` rather than maintained by hand, so a scan that found no posts renders an EMPTY page that still answers 200: assert the count, not the status.
- `/blog/folk-technology` -> 200 (a post reached by its slug, the segment `<RoutesDir/>` derives from the post's own `slug` frontmatter rather than its `<number>-` filename)
- `/blog/post-6` -> **301** to `/blog/folk-technology`. Every post was renamed from `post-<n>` to a slug, and `site/main.bsx` keeps the old urls alive with a block of `<Redirect path="post-<n>" redirect="<slug>"/>`. That block is part of the entry document the box loads out of the repo store, so it is exactly as deployable-and-breakable as a route: sweep all fifteen (`for n in $(seq 1 15); do curl -o /dev/null -w '%{http_code} %{redirect_url}\n' "$BASE/blog/post-$n"; done`) and assert every one is a 301 to a slug that itself answers 200. A `<Redirect/>` carries no `PageRoute`, so it is absent from the sidebar and from `RouteIndex`, and nothing but this check covers it.
...
The driver lives beside this skill, in `.agents/skills/infra-deploy/` (it is COMMITTED there on purpose: `.agents/tmp` is gitignored, and an earlier copy kept there silently vanished between deploys). `ssh.sh <HOST> <PORT>` runs the whole check (single session + two-client multi-tenancy) and `ssh_pty.py <HOST> <PORT> '<SCRIPT>'` is the underlying pty driver, python3 stdlib only.

- local: `.agents/skills/infra-deploy/ssh.sh 127.0.0.1 <ssh_port>` (default 8339; trust the serve output)
- dev/prod: `.agents/skills/infra-deploy/ssh.sh app.dev.beet.org 22` / `.agents/skills/infra-deploy/ssh.sh app.beet.org 22`
...
- The image scrolled into view (31 `k:down`), since it sits well below the fold. The post itself is reached by label (`t:Folk Technology`, post-6), for the same reason the navigation recipe is.
...
REGRESSION GUARD (fixed): every click inside the open sidebar overlay landed on the item TWO ROWS BELOW at 80x24. The focus ring is a border, so pressing a link moved focus off the hamburger, its ring dropped, the header shrank by two rows and the overlay rose with it before the release arrived; the release was re-hit-tested against the new geometry and reached the wrong link. `pointer_input` now delivers a release on the pressed cell to the pressed entity (`crates/beet_ui/src/render/charcell/hit_test.rs`, covered by `release_lands_on_the_pressed_entity_after_a_reflow`). The in-process twin is `cargo test --test bsx_site_tui --features router,style,markdown,fs,tui_server,quickjs,testing` (7 tests; `quickjs` is a required feature, since the counter's `bx:click` verbs are scripts, and without it cargo skips the target rather than failing it).
...
The site records analytics for every transport: a server `Request` event per routed request, a `PageView` (with dwell) per web/terminal page visit, and `Click` / `Scroll` / `Error` events from the web client beacon. Events buffer into immutable gzip NDJSON segments. Verify the visits from checks b-d landed in the store and that prior segments and rollups remain visible. Query with the `beet analytics` subcommand, mounted bare in `site/main.bsx`: it reads the store the serving router's `AnalyticsConfig` writes to, since the document loads whole and that config's `StoreRef` resolves the stage and service access of the launch (built with `aws_sdk` so `--service-access=remote` reaches the live S3 bucket):

- local: `cargo run -p beet-cli -- --main=site analytics summary` (the store the site writes to, `target/stores/beet-site--dev--analytics`)
- dev/prod: `cargo run -p beet-cli --features infra -- --main=site --stage=<stage> --service-access=remote analytics summary` (the same config, resolved remote), or name a store outright with `--store s3://beet-site--<stage>--analytics`

Recipe (run around the b-d checks so the delta is attributable):

1. BASELINE: query once before the b-d checks and record the total (`N events: ...`). Expect a non-zero baseline even on a brand-new stage: port 80/443 is open to the internet, so scanners start recording `Request` events within a minute of the box serving (a fresh dev box read `288 events`, every one of them a `Web` `Request` for `/wp-admin` and friends, twelve minutes after the deploy returned). What matters is the DELTA, not the floor. That noise is also the earliest free proof that geoip is live, since a scanner's country resolves exactly like a real visitor's.
...
3. DELTA: query again and assert, from the summary:
   ...
   - for dev/prod (geoip enabled in the deploy build), a country appears under `countries` for the web visits. The geoip database (`assets/databases/country.mmdb`, ~8MB) is gitignored and workspace-owned, so it reaches the repo store by being borrowed into the staging dir by the `<DirCopy/>` under the deploy's `<RepoStage>`: run `just beet-shared pull` AND `just site-shared pull` before deploying (the deploy mirrors the staged `site/` whole, so an unhydrated `site/assets` would publish a site without its assets), and if `countries` is empty verify `country.mmdb` is present under `s3://beet-site--<stage>--repo/<deploy id>/repo/assets/databases/`. The lookup is switched on by the `GeoIpDb` spread on the serving `<Router>` in `site/main.bsx`, which reads that path out of the repo store like any other asset; without it every country is empty by authorship, and a declared-but-absent database logs `geoip: no country database at ..` on boot.

REGRESSION GUARD (fixed): the deploy and runtime once derived the analytics resource name independently, so a missing app name let the site serve while every event write targeted a resource the deploy never created. The name is no longer derived twice: `site/main.bsx` declares the bucket once and both sides compose it through the one resolved `Stack` (pinned by `one_declaration_names_the_bucket_for_both_sides` in `crates/beet_extra/src/infra/templates.rs`). If a summary reports `0 events`, inspect `/var/log/beet-site.log` for a segment-write error naming the bucket and region first.

### f. the nightly rollup (archive, aggregate, compact)

The analytics segment namespace does not grow forever. A `<ScheduledJobBlock label="rollup-daily"/>` invokes the `rollup` lambda at 03:00 UTC, which boots the entry's `jobs` verb out of the repo store, at the prefix its own deploy published, and dispatches `rollup` into its router. Each run merges every complete day's segments with its existing `analytics/raw/<date>.ndjson.gz` archive in `beet-site--<stage>--archive`, writes that day's deterministic `AnalyticsRollup` rows into the JSON-over-blob `analytics/rollup/` prefix of that same `beet-site--<stage>--analytics` bucket, reads both outputs back, and only then deletes the source segments from `analytics/raw/segments/`. The daily archive is the compact primary raw history; rollups are kept and can be re-derived from it.

- `beet analytics summary` now reports over BOTH stores: aggregates for the days they cover, raws for the recent window they do not. `--raw-only` skips the aggregates, `--rollup <uri>` overrides the aggregate store (it defaults to the events store, the site keeping both under one bucket).
...
- The first ordinary run compacts every complete day represented by segments and repairs aggregate rows for archived-but-uncovered days. Re-invoking continues where a timeout stopped: a failed archive or rollup verification leaves every source segment intact. `--full` (`{"kind":"beet.scheduled_invoke.v1","method":"Post","path":"rollup?full=true"}`) re-derives every archived complete day for a new aggregate schema; deterministic archive paths, aggregate IDs, and event-ID deduplication make it idempotent.
- WATCH FOR: `deleted` may be nonzero only after the same run reports and verifies an archive and aggregate rows for those dates. A failed run must leave its source segments present; treat missing segments without both outputs as a bug, not an operational hiccup.
- ON DEV, an invoke commonly reports `0 days` because `AnalyticsRollupRun::dates` never covers today and a dev stack exists for less than one UTC day. That proves the function boots and reaches all three stores; the focused fs compaction tests prove archive/write/delete ordering. For a live end-to-end check, upload a valid yesterday segment under `analytics/raw/segments/<date>/<writer-uuid>/<timestamp>-<sequence>.ndjson.gz`, invoke, then assert the daily archive and aggregate objects exist and the source segment is gone. Re-invoking must report `0 days`. Valid means every event's `timestamp` falls on the path's date: the run checks each one (`AnalyticsSegment::read`) and a mismatch fails the whole invoke with a 500, leaving the segment intact, so a today segment merely rekeyed under yesterday is refused. Build the seed by shifting a real segment's timestamps back one day (gzip NDJSON, one event per line) and giving its events fresh ids.
- ON PROD, once the schedule has been firing, `0 days` is the normal daytime result because 03:00 UTC compacted yesterday. Verify coverage independently: every completed date formerly represented by segments has `analytics/raw/<date>.ndjson.gz` in the archive bucket and a site-scope aggregate object under `analytics/rollup/`. Any leftover complete-day segment should be consumed by the next run.
- A long-dwell `PageView` can appear in more than one segment with the same ID. Compaction keeps the newest timestamp before archiving and aggregating, so retries and overlapping writes do not double-count it.

Cross-check the raw namespace with `aws s3 ls s3://beet-site--<stage>--analytics/analytics/raw/segments/ --recursive` and completed days with `aws s3 ls s3://beet-site--<stage>--archive/analytics/raw/`; the CLI remains the authoritative event total because each object contains many events and combines segments with rollups. `BlobStore::GET_ALL_CONCURRENCY` bounds remote reads. An automated in-process version of the web half of this flow (http request -> request event, beacon -> page view, prior events retained, the beacon endpoint skipped) lives in `tests/beet_site_analytics.rs`; run it with `cargo test --test beet_site_analytics --features "router,json,fs,testing"` for a fast pre-deploy check of the wiring. The terminal page-view path is unit-tested in `beet_router/src/navigate/navigator.rs`.
...
EYEBALL the plan for three things specifically: that every existing S3 bucket appears only as a refresh or deliberate in-place update; that the repo store, archive store, instance, and DNS are never replaced (the one exception is a deliberate machine-config change to the instance's `user_data`, which replaces the box); and that DNS changes are exactly the ones expected. During the one-time storage migration, the two Dynamo table destroys and the two same-label S3 bucket creates are deliberate: raw events may expire and rollup rows are re-derived from the archive.

A BASELINE REDEPLOY OF UNCHANGED MAIN PLANS EXACTLY ONE CHANGE. After the storage migration, a code-only deploy reads `Plan: 0 to add, 1 to change, 0 to destroy`: the rollup lambda's `s3_key`, `source_code_hash` and its two `BEET_DEPLOY_*` variables. Everything else, the app, analytics, and archive buckets, the instance, and all three Cloudflare records, appears as a `Refreshing state...` line and nothing more. The box itself is NOT in the plan at all: the binary reaches it through the release pointer, so `uptime` on the box spans previous deploys and an instance uptime that resets on a code-only deploy means `user_data` moved.
...
1. **Hydrate both asset trees** (`just beet-shared pull` for `./assets`, `just site-shared pull` for `./site/assets`). The deploy stages `site/` whole and mirrors the stage, so it publishes whatever the checkout holds. `<RepoStage>` refuses a missing or empty source rather than emptying the store, but a *stale* tree stages happily and silently ships old assets.
2. **The apply runs once per layer, and the order matters.** `<TofuApply layer="storage"/>` brings up the stores (the addresses blocks declare under the `storage` layer, an overridable per-block field), `<RepoStage>` and `<RepoSync/>` fill the repo store, then a bare `<TofuApply/>` converges the whole stack and replaces the box against content that is already published. A single apply built the instance first, and it booted with `no entry document found in the --repo backend`. Naming a layer no block declares is a loud error, never a silent skip. Since the box is replaced rather than rolled, there is no health gate and no rollback: `LightsailWatch` is a CloudWatch log tail, not a readiness check, so re-check the site after the deploy returns, not during.
3. **Diff the rendered configs before/after** whenever `beet_infra` or the deploy render changes: `.agents/skills/infra-deploy/render_all.sh before` on the clean tree FIRST, then after the change `.agents/skills/infra-deploy/render_all.sh after .agents/tmp/render/before` renders all eight stacks and diffs each against the baseline. `HASH-ONLY` means only `source_code_hash` moved, ie a rebuilt binary shipping as a code-only deploy, not a config change. Any other diff must be deliberate and explainable line by line (invariants: no existing physical resource name changes, no replacement of an existing resource). The dumps are disposable and live in `.agents/tmp/`; nothing is committed.
