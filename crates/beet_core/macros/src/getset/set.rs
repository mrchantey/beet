...
pub(crate) fn impl_set(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
...
fn generate_field(field: &syn::Field, config: &FieldConfig) -> TokenStream {
	...
	if config.unwrap_option {
		if let Some(inner_ty) = option_inner_type(ty) {
			...
			return quote! {
				#(#doc)*
				#[inline(always)]
				#vis fn #fn_name(&mut self, val: #param) -> &mut Self {
	...
	if effective_use_into(ty, config) {
		quote! {
			#(#doc)*
			#[inline(always)]
			#vis fn #fn_name(&mut self, val: impl Into<#ty>) -> &mut Self {
