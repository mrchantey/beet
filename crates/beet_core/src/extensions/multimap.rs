...
//! the `#[reflect(Default)]` attribute. A leaf field is parsed through its type's
//! [`LiteralParser`] entry, so every type with an authored spelling works here and
//! in markup identically, `Duration` and `GlobFilter` as much as `u16`. Field types
//! can be:
//! - `bool` - parsed from "true"/"false" strings, or bare presence as a flag. This
//!   is a MAP-shape rule rather than a parse one, so it runs ahead of the parser
//!   lookup and a bare `--watch` stays `Some(true)`
//! - any type carrying a [`LiteralParser`] - `String`, `SmolStr`, the numeric
//!   primitives, `Duration`, `Timestamp`, `GlobFilter`, and whatever a downstream
//!   crate registers
//! - `Vec<T>` where `T` has one - all values for a key, each parsed as an item
...
//!
//! A field whose type has no entry and matches no structural rule is an error
//! naming the type, never a silent default.
//!
...
//!
//! let params: QueryParams = map.parse_reflect().unwrap();
...
fn build_field_value(
	...
	let option_inner = field_type_info
	...
	// (one value a `Value::Str`, a repeated key a `Value::List`). A parser that
	// declines, or a type with no entry, falls to the structural rules below.
	if let Some(values) = map.get_vec(field_name) {
		if let Some(parsed) = parse_leaf_literal(values, field_type_id)? {
			return Ok(Some(parsed));
		}
		if let Some(inner) = option_inner
			&& let Some(parsed) = parse_leaf_literal(values, inner)?
		{
			return Ok(Some(wrap_some(parsed)));
		}
	}
	...
	// a list-typed field takes EVERY value under its key, each parsed into the
	// item type, so `--tag a --tag b` is `["a","b"]`.
	if let Some(TypeInfo::List(list_info)) = field_type_info
		&& let Some(item_info) = list_info.item_info()
		&& let Some(parser) = LiteralParser::get(item_info.type_id())
	{
		let mut list = DynamicList::default();
		for value in map_item {
			let Some(item) = parser.parse(&Value::str(value.as_str()))? else {
				return unsupported_field(field_name, Some(item_info));
			};
			list.push_box(item);
		}
		return Ok(Some(Box::new(list)));
	}
	unsupported_field(field_name, field_type_info)
}
...
fn unsupported_field<T>(
	...
	bevybail!(
		"unsupported field type for '{field_name}': no authored form for `{type_path}`, expected a bool, a nested struct, or a type carrying a `LiteralParser`"
	)
}
...
mod test {
	...
	fn optional_leaf_reports_malformed() {
		...
		map.parse_reflect::<WithOptionalLeaves>()
			.unwrap_err()
			.to_string()
			.xpect_contains("invalid number \"nope\"");
	}
