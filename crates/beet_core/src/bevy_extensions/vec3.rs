...
pub impl Vec3 {
	...
	fn random_in_cube_signed(rng: &mut impl rand::RngExt) -> Self {
		Vec3::new(
	...
	fn random_in_cube(rng: &mut impl rand::RngExt) -> Self {
		Vec3::new(
	...
	fn random_on_sphere(rng: &mut impl rand::RngExt) -> Self {
		// `f32::sin`/`cos` are std-only; `bevy::math::ops` uses libm under no_std.
		use bevy::math::ops;
	...
	fn random_in_sphere(rng: &mut impl rand::RngExt) -> Self {
		// `f32::sin`/`cos`/`powf` are std-only; `bevy::math::ops` uses libm under no_std.
		use bevy::math::ops;
		let theta = rng.random_range(0.0..core::f32::consts::TAU);
		let phi = rng.random_range(0.0..core::f32::consts::PI);
		let r = ops::powf(rng.random_range(0.0f32..1.0), 1. / 3.);
		Vec3::new(
