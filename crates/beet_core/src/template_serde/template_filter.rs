...
/// A filter used to control which types can be added to a [`DynamicTemplate`].
///
/// This filter _can_ be used more generically to represent a filter for any type;
/// its intended usage with [`DynamicTemplate`] only considers [components] and [resources].
/// Adding types that are not a component or resource has no effect with `DynamicTemplate`.
///
/// [`DynamicTemplate`]: crate::prelude::DynamicTemplate
...
pub enum TemplateFilter {
	...
	///
	/// [`Denylist`]: TemplateFilter::Denylist
	/// [`Allowlist`]: TemplateFilter::Allowlist
	/// [Allowing]: TemplateFilter::allow
	/// [denying]: TemplateFilter::deny
	#[default]
	Unset,
...
impl TemplateFilter {
	...
	///
	/// If the filter is [`Unset`](TemplateFilter::Unset) this always returns `true`.
	...
	///
	/// If the filter is [`Unset`](TemplateFilter::Unset) this always returns `false`.
	...
	/// Returns an iterator over the items in the filter, empty if [`Unset`](TemplateFilter::Unset).
	...
	/// Returns the number of items in the filter, zero if [`Unset`](TemplateFilter::Unset).
	...
	/// Returns true if there are zero items in the filter, ie [`Unset`](TemplateFilter::Unset).
...
impl IntoIterator for TemplateFilter {
	type Item = TypeId;
...
mod test {
	...
	#[crate::test]
	fn adds_to_list() {
		let filter = TemplateFilter::default().allow::<i16>().allow::<i32>();
		filter.len().xpect_eq(2);
		filter.is_allowed::<i16>().xpect_true();
		filter.is_allowed::<i32>().xpect_true();

		let filter = TemplateFilter::default().deny::<i16>().deny::<i32>();
		filter.len().xpect_eq(2);
