...
impl MockModelProvider {
	...
	fn extract_input_text(request: &o11s::RequestBody) -> String {
		match &request.input {
			...
			o11s::request::Input::Items(items) => {
				for item in items {
					if let o11s::request::InputItem::Message(msg) = item {
	...
	fn create_text_response(text: &str) -> o11s::response::Body {
		o11s::response::Body {
			...
			output: vec![o11s::OutputItem::Message(o11s::Message {
				id: next_id("msg"),
	...
	fn create_function_call_response(
		...
		o11s::response::Body {
			...
			instructions: None,
			model: Some("mock".to_string()),
			output: vec![o11s::OutputItem::FunctionCall(o11s::FunctionCall {
				id: next_id("fc"),
				call_id,
				name: name.to_string(),
		...
		Box::pin(async move {
			...
			bevy::tasks::IoTaskPool::get()
				.spawn(async move {
					...
					let _ = sender
						.send(Ok(o11s::StreamingEvent::ResponseCreated(
					...
					let _ = sender
						.send(Ok(o11s::StreamingEvent::ResponseCompleted(
...
mod test {
	...
	async fn echoes_input_without_tools() {
		let provider = MockModelProvider::default();
		let body =
			o11s::RequestBody::new("mock").with_simple_input("Hello world!");

		let response = provider.send(body).await.unwrap();

		response.status.xpect_eq(o11s::response::Status::Completed);
	...
	async fn calls_first_tool_when_present() {
		let provider = MockModelProvider::default();
		let tool = o11s::FunctionToolParam::new("greet").with_parameters(
			serde_json::json!({
				"type": "object",
				"properties": {
					"name": { "type": "string" },
	...
	async fn custom_response_overrides_echo() {
		...
		let body = o11s::RequestBody::new("mock").with_simple_input("Hello!");
	...
	async fn streaming_calls_tool() {
		let provider = MockModelProvider::default();
		let tool = o11s::FunctionToolParam::new("test_tool").with_parameters(
			serde_json::json!({
				"type": "object",
		...
		while let Some(result) = stream.next().await {
			...
			if let o11s::StreamingEvent::OutputItemAdded(ev) = event {
				if let Some(o11s::OutputItem::FunctionCall(fc)) = ev.item {
