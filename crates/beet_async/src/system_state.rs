...
/// Placeholder cell carried by exclusive bridge requests (beet patch).
///
/// Exclusive futures access the raw `&mut World` directly inside
/// `ScopedStatic::try_with`, so there is no deferred `SystemState` to apply.
/// They still need *a* cell so the existing driver can wake/serve them
/// unchanged; this one's `apply` is a no-op.
pub(crate) struct NoopSystemStateCell;

impl ErasedSystemStateCell for NoopSystemStateCell {
