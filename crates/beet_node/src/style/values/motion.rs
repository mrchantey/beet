...
impl CssValue for EaseFunction {
	fn to_css_value(&self) -> String {
      ...
      EaseFunction::SmootherStep | EaseFunction::SmootherStepIn | EaseFunction::SmootherStepOut =>
      "linear(0, 0.002, 0.015, 0.05, 0.11, 0.21, 0.35, 0.5, 0.65, 0.79, 0.89, 0.95, 0.985, 0.998, 1)".to_string(),
      // Bounce Out: Approximating the "hitting the floor" look
      EaseFunction::BounceOut =>
      "linear(0, 0.36, 0.73, 1, 0.81, 1, 0.94, 1, 0.98, 1)".to_string(),
      // Elastic Out: Approximating the springy overshoot
      EaseFunction::ElasticOut =>
      "linear(0, 1.32, 0.87, 1.05, 0.98, 1.01, 1)".to_string(),
      // Bounce/Elastic In/InOut usually just reverse these or use more points
      ...
      // Fallback for types not explicitly mapped to linear points yet
