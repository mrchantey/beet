...
fn insert_options_struct<T: 'static>(
	...
	let Some(registration) = type_registry.get_mut(std::any::TypeId::of::<T>())
		...
		log::warn!("Attempting to set default inspector options for {}, but it wasn't registered in the type registry.", std::any::type_name::<T>());
		return;
	};
	...
	if registration.data::<ReflectInspectorOptions>().is_none() {
		...
		for (variant, field, data) in fields {
			...
			let field_index = match info.variant_at(variant_index).unwrap() {
				bevy::reflect::VariantInfo::Struct(strukt) => {
...
pub fn register_default_options(_type_registry: &mut TypeRegistry) {
	// insert_options_enum::<Color>(type_registry, &[
	...
	// insert_options_struct::<bevy_render::view::ColorGrading>(type_registry, &[
	// 	(
	// 		"exposure",
	...
	// {
	// 	insert_options_struct::<bevy_pbr::AmbientLight>(type_registry, &[(
	// 		"brightness",
	// 		&NumberOptions::<f32>::positive(),
	// 	)]);
	// 	insert_options_struct::<bevy_pbr::PointLight>(type_registry, &[
	// 		("intensity", &NumberOptions::<f32>::positive()),
	// 		("range", &NumberOptions::<f32>::positive()),
	// 		("radius", &NumberOptions::<f32>::positive()),
	// 	]);
	// 	insert_options_struct::<bevy_pbr::DirectionalLight>(type_registry, &[
	// 		("illuminance", &NumberOptions::<f32>::positive()),
	// 	]);
	// 	insert_options_struct::<bevy_pbr::StandardMaterial>(type_registry, &[
	// 		(
	...
	// 	]);
	// 	insert_options_enum::<bevy_pbr::ClusterConfig>(type_registry, &[
	// 		("FixedZ", "z_slices", &NumberOptions::<u32>::at_least(1)),
	// 		(
	// 			"XYZ",
	// 			"dimensions",
	// 			&NumberOptions::<bevy_math::UVec3>::at_least(
	// 				bevy_math::UVec3::ONE,
	// 			),
	// 		),
	// 	]);
	...
	// insert_options_struct::<bevy_time::Virtual>(type_registry, &[
