...
mod test {
	...
	fn works() {
		...
		stat_map.map.len().xpect_eq(1);
		stat_map.map.get(&StatId(0)).unwrap().xpect_eq(stat.clone());

		stat_map.add_stat(stat.clone());
		stat_map.map.len().xpect_eq(2);
		stat_map.map.get(&StatId(1)).unwrap().xpect_eq(stat);
	}
}
