//! Composable tool wrapping via the wrap combinator.
//!
//! The [`IntoWrapTool`] trait lets you wrap a tool handler with
//! middleware-style logic. The wrapper function receives the outer
//! input plus a [`Next`] handle for calling the inner handler.
//!
//! ## Examples
//!
//! ```rust
...
//!
//! let handler = tool(
//!     (async |input: String, next: Next<i32, i32>| -> Result<String> {
//!         let parsed: i32 = input.parse().map_err(|err| bevyhow!("{err}"))?;
...
pub trait IntoWrapTool<WrapIn, WrapOut, InnerIn, InnerOut>: Sized {
	...
	where
		Inner: IntoToolHandler<InnerM, In = InnerIn, Out = InnerOut>,
...
mod test {
	...
	#[test]
	fn wrap_transforms_input_and_output() {
		AsyncPlugin::world()
			.spawn(tool(serde.wrap(double)))
			.call_blocking::<String, Result<String>>("21".into())
			.unwrap()
			.unwrap()
	...
	#[test]
	fn wrap_modifies_inner_input_and_output() {
		AsyncPlugin::world()
			.spawn(tool(
				(async |input: i32, next: Next<i32, i32>| -> Result<i32> {
					let inner_out = next.call(input * 10).await?;
