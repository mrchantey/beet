...
fn diff_by_format(
	...
	match format {
		FileFormat::Markdown => MarkdownDiffer::new(text).diff(entity),
		FileFormat::Html => HtmlDiffer::new(text).diff(entity),
...
///
/// Reads the file from disk using [`fs_ext::read_to_string`],
/// detects the format from the extension, and applies the
/// appropriate parser as children.
pub fn load_mime_file_content(world: &mut World) {
	...
	for (entity, path) in to_load {
		...
		match fs_ext::read_to_string(&path.into_abs()) {
			Ok(text) => {
				if let Err(err) =
					diff_by_format(format, &text, world.entity_mut(entity))
				{
					cross_log_error!(
