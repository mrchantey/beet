...
impl<T: 'static + Send + Sync + Clone, E: 'static + Send + Sync + Clone> Default
	for EndInDuration<T, E>
where
	End<T, E>: Default,
{
...
impl<T: 'static + Send + Sync + Clone, E: 'static + Send + Sync + Clone>
	EndInDuration<T, E>
{
	/// Specify the payload and duration
	pub fn new(event: End<T, E>, duration: Duration) -> Self {
		Self { event, duration }
	}
}
...
pub(crate) fn end_in_duration<
	T: 'static + Send + Sync + Clone,
	E: 'static + Send + Sync + Clone,
>(
	mut commands: Commands,
	mut query: Populated<
		(Entity, &RunTimer, &mut EndInDuration<T, E>),
		With<Running>,
	>,
) {
