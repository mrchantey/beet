	...
	fn apply_template(&self, node: &mut RsxNode) -> TemplateResult<()> {
		...
		if let Some(template) = self.templates.get(&location) {
			...
			*node = template
				...
				.clone()
				.into_rsx_node(&mut RustyPartMap::collect(node))
...
mod test {
	...
	fn split_join() {
		...
		let page_template = RsxTemplateNode::from_rsx_node(&node1).unwrap();
	...
	fn nested_templates() {
		...
		let template = RsxTemplateNode::from_rsx_node(&node).unwrap();
