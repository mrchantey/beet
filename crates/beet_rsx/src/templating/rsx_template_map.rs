...
mod test {
	...
	fn split_join() {
		...
		let html1 = node1.clone().bpipe(RsxToHtmlString::default()).unwrap();
		let template = node1.bref().bpipe(NodeToTemplate).unwrap();
	...
	fn rsx_template_match_simple() {
		...
		expect(&node1.bref().bpipe(NodeToTemplate).unwrap())
			.not()
	...
	fn nested_templates() {
		...
		let template = node.bref().bpipe(NodeToTemplate).unwrap();
		...
		let html1 = node.clone().bpipe(RsxToHtmlString::default()).unwrap();
		let html2 = node
			.bpipe(&map)
			.unwrap()
			.bpipe(RsxToHtmlString::default())
	...
	fn ignores_exterior_roots() {
		...
		expect(comp.clone().bpipe(&map)).to_be_err();
		// exterior root, ok
		expect(
