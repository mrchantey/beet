...
- Workload-named buckets such as `analytics` and `analytics-rollup` hold runtime-written primary data. They set `runtime_write=true` and `deploy_versioned=false`; analytics segments delete only after the archive and JSON-over-blob rollups verify.
