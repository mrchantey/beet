...
/// The deployable artifact label a block declares, inserted from the block's
/// own field on add (`#[component(on_add = ArtifactLabel::on_add::<MyBlock>)]`)
/// so artifact collection is a plain query over `(ArtifactLabel,
/// BuildArtifact)` pairs.
#[derive(Debug, Clone, Component)]
#[component(immutable)]
pub struct ArtifactLabel(pub SmolStr);

impl ArtifactLabel {
	/// Component hook: derive the label from the block component on the same
	/// entity, ie `#[component(on_add = ArtifactLabel::on_add::<LambdaBlock>)]`.
	pub fn on_add<T: Component>(mut world: DeferredWorld, cx: HookContext)
	where
		for<'a> &'a T: Into<ArtifactLabel>,
	{
		let entity = cx.entity;
		world.commands().queue(move |world: &mut World| -> Result {
			let label: ArtifactLabel =
				world.entity(entity).get_or_else::<T>()?.into();
