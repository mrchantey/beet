...
fn openai_message_request(
	...
	commands.run_local(async move |queue| {
		...
		let mut stream = req.send().await?.event_source_raw().await?;
		...
		while let Some(ev) = stream.next().await {
			...
			let Ok(body) = serde_json::from_str::<Value>(&ev.data) else {
				eprintln!("failed to parse event data as json: {}", ev.data);
				continue;
			};
			...
			match body.field_str("type")? {
				...
				"response.output_item.done" => {
					...
					match body["item"]["type"].to_str()? {
						"image_generation_call" => {
							let ext = body["item"]["output_format"].to_str()?;
				...
				"response.completed" => {
					let input_tokens =
						body["response"]["usage"]["input_tokens"].to_u64()?;
					let output_tokens =
						body["response"]["usage"]["output_tokens"].to_u64()?;
