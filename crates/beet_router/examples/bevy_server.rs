...
async fn handle_request(
	...
	world.with_resource::<RequestCounter>(|mut counter| {
		counter.0 += 1;
	});

	// Get current count
	let count = world.resource::<RequestCounter>().await.0;
...
fn start_server_system(mut async_commands: AsyncCommands) {
			...
			bevy::tasks::IoTaskPool::get()
				.spawn(async move {
					...
					if let Err(err) = http1::Builder::new()
						...
						.await
					{
						// Don't log header timeouts as errors - they're normal for keep-alive connections
						if err.is_timeout()
							&& err.to_string()
...
fn main() -> Result {
	...
	app.add_plugins((DefaultPlugins, AsyncPlugin))
		.insert_resource(RequestCounter(0))
		.add_systems(Startup, start_server_system);
