...
//! a site load reads every `.bsx` under the store's `templates/` subdirectory and
//! registers each by its module path, so the same templates resolve whether the
...
/// Read every `.bsx` source under the site store's `templates/` directory as
/// `(path, source)` pairs (paths relative to that directory). Async (store I/O),
/// so a load path awaits it off the runtime rather than blocking. A site with no
...
/// [`BsxTemplateRegistry`] by its module path, then refresh the BSX schemas. The
/// synchronous world-mutating tail of a site-template load, applied once
...
pub fn register_site_templates(
	...
	for (path, source) in sources {
		registry.insert_source_from_path(&path, &source)?;
	}
