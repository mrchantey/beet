...
impl ParseDirRoutes {
	...
	pub fn build_string(config: &CollectRoutes, path: &Path) -> Result<String> {
		...
		// maybe more consistent to just use file!()
		// let path_str = path.to_string_lossy();

		let use_crate_as_pkg_name = if let Some(pkg_name) = &config.pkg_name {
			let pkg_name = pkg_name.parse::<TokenStream>().unwrap();
			quote! {
