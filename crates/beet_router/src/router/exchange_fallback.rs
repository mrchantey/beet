...
mod test {
	...
	async fn first_pass_returns() {
		AsyncPlugin::world()
			.spawn((Router::exchange_fallback(), children![
	...
	async fn no_match_returns_not_found() {
		AsyncPlugin::world()
			.spawn((Router::exchange_fallback(), children![Action::<
				Request,
	...
	async fn works_with_router() {
		(AsyncPlugin, RouterPlugin)
			.into_world()
			.spawn((Router::with_defaults(), children![Router::route(
				"fb",
				(Router::exchange_fallback(), children![Action::<
					Request,
					Outcome<Response, Request>,
				>::new_pure(
					|_cx: ActionContext<Request>| {
