...
//! is the `Startup` system that drives it for a running app; and the `beet`
//! facade's `launch::app` composes that plugin with `BeetPlugins` into the one
//! app body every beet binary runs.
pub mod entry_build;
