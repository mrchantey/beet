...
mod test {
	...
	async fn works() {
		...
		expect(&html[0].1.render()).to_be("<!DOCTYPE html><html><head></head><body><div><h1 data-beet-rsx-idx=\"3\">Beet</h1>party time dude!</div></body></html>");
		expect(&html[1].0.path.to_string_lossy()).to_be("/");
		expect(&html[1].1.render()).to_be("<!DOCTYPE html><html><head></head><body><div><h1 data-beet-rsx-idx=\"3\">My Site</h1>party time!</div></body></html>");
	}
}
