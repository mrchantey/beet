...
//!
//! `<Drive>` (in `beet_action`) writes a body's commanded [`DifferentialDrive`] via
...
//! wheels, so the *same* `<Drive>` square patrol drives the on-screen fox and the
//! real robot.
...
/// A wgpu body driven by a [`DifferentialDrive`] (the commanded motion a `<Drive>`
...
/// unit, so the same `<Drive linear=60 angular=90>` square patrol that crawls a 6 cm
/// square on the robot walks a 60-unit square here. `#[require(DifferentialDrive)]`
/// declares the command on the body at spawn, satisfying `<Drive>`'s requirement.
