...
pub async fn streaming_response(provider: impl ModelProvider) {
	...
	let mut final_response: Option<o11s::ResponseBody> = None;
...
pub async fn tool_calling(provider: impl ModelProvider) {
	...
	if let Some(status) = fc.status {
		status.xpect_eq(o11s::FunctionCallStatus::Completed);
	}
...
pub async fn image_input(provider: impl ModelProvider) {
	...
	let msg = o11s::request::MessageParam::with_parts(
		o11s::MessageRole::User,
		vec![
			o11s::ContentPart::input_text(
				"What color is this image? Answer in one word, either 'red' 'green' or 'blue'",
			),
			o11s::ContentPart::InputImage(o11s::InputImage::from_base64(
				"image/png",
				image_b64,
			)),
		],
	...
	let body = o11s::RequestBody::new(provider.default_small_model())
		.with_input_items(vec![o11s::request::InputItem::Message(msg)]);
...
pub async fn multi_turn_conversation(provider: impl ModelProvider) {
	let body = o11s::RequestBody::new(provider.default_small_model())
		.with_input_items(vec![
			o11s::request::InputItem::Message(
				o11s::request::MessageParam::user("My name is Alice."),
			),
			o11s::request::InputItem::Message(
				o11s::request::MessageParam::assistant(
					"Hello Alice! Nice to meet you. How can I help you today?",
				),
			),
			o11s::request::InputItem::Message(
				o11s::request::MessageParam::user("What is my name?"),
			),
		]);
