...
#[extend::ext]
pub impl Request {
	/// An ECS version of [`Request::send`], converting the request
	/// into an [`ExchangeStart`] entity event, and awaiting the
	/// call to [`ExchangeEnd::send`].
	fn exchange(
		self,
		...
		async move {
			...
			let start = ExchangeStartInner::new(self, send);
			handler.handle(start);
			recv.recv().await.expect("Exchange sender was dropped")
		}
	}
}
...
pub trait ExchangeHandler {
	fn handle(self, start: ExchangeStartInner);
}

impl ExchangeHandler for &AsyncEntity {
	fn handle(self, start: ExchangeStartInner) { self.trigger(start); }
}
...
pub struct ExchangeStart {
	#[event_target]
	target: Entity,
	inner: Arc<Mutex<Option<ExchangeStartInner>>>,
}
...
impl ExchangeStart {
	...
	pub fn new(
		...
		Self {
			target,
			inner: Arc::new(Mutex::new(Some(ExchangeStartInner {
				request,
				end: ExchangeEnd {
	...
	pub fn take(&self) -> Result<ExchangeStartInner> {
		let mut inner = self.inner.lock().unwrap();
		inner
			.take()
...
pub struct ExchangeStartInner {
	pub request: Request,
	pub end: ExchangeEnd,
}

impl ExchangeStartInner {
	/// Create a new [`ExchangeStart`],
	/// providing the request and a channel [`Sender`]
	/// for when the exchange is complete
...
#[cfg(feature = "nightly")]
impl FnOnce<(Entity,)> for ExchangeStartInner {
...
mod test {
	...
	fn unhandled_panics_raw() {
		...
		world
			.spawn_empty()
			.trigger(ExchangeStartInner::new(Request::get("foo"), send));
	}
