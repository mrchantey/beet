...
impl Plugin for BaseServerPlugin {
	fn build(&self, app: &mut App) {
		...
		app.add_systems(
			PreUpdate,
			(replicate_cursors, replicate_players)
...
pub(crate) fn init(mut commands: Commands) { commands.start_server(); }
...
pub(crate) fn replicate_players(
	...
	replicated_players: Query<
		(Entity, &Replicated),
		(With<PlayerPosition>, Added<Replicated>),
	>,
) {
	for (entity, replicated) in replicated_players.iter() {
		...
		if let Some(mut e) = commands.get_entity(entity) {
			...
			e.insert((
				...
				OverrideTargetComponent::<PrePredicted>::new(
...
pub(crate) fn replicate_cursors(
	...
	replicated_cursor: Query<
		(Entity, &Replicated),
		(With<CursorPosition>, Added<Replicated>),
	>,
) {
