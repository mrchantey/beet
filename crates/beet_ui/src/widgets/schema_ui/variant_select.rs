...
//! [`ValueRebuild`](super::value_rebuild::ValueRebuild) the two sit in.
use crate::prelude::*;
...
///
/// Scoped to the widget set rather than to `schema_ui`, only because
/// [`write_selected_variant`] is registered by
/// [`FormPlugin`](crate::prelude::FormPlugin) and so names it in a signature the
/// `controls` side can see.
#[derive(Component)]
...
pub(super) fn variant_select(
	...
	let variants = schema
		...
		.iter()
		.map(|variant| {
			(variant.name.clone(), variant.default_value_in(resolver))
		})
		.collect::<Vec<_>>();
