...
/// Resolve uppercase [`Element`] nodes in an already-parsed tree (eg one a
/// markdown/MDX front-end built) as BSX components or templates.
///
/// MDX is markdown prose plus BSX markup: the markdown parser emits an embedded
/// `<MyTemplate>` as a plain uppercase [`Element`], and this pass reconstructs
/// each into a [`BsxElement`] and rebuilds it through the uppercase dispatch, so
/// a `<MyTemplate>` resolves for free without a second resolver.
pub fn resolve_mdx_templates(world: &mut World, root: Entity) -> Result {
	// collect uppercase element entities depth-first (deepest first, so a parent
	// rebuild does not invalidate an already-resolved child).
	let mut targets = Vec::new();
	collect_uppercase_elements(world, root, &mut targets);
	let registry = world
		.get_resource::<BsxTemplateRegistry>()
		.cloned()
		.unwrap_or_default();
	for entity in targets.into_iter().rev() {
		let Some(node) = reconstruct_element(world, entity) else {
			continue;
		};
		// despawn the placeholder's children and strip its markers, then rebuild.
		...
		let template =
			BsxTemplate::new(vec![BsxNode::Element(node)], registry.clone());
		world.entity_mut(entity).insert_template(template);
	}
	Ok(())
}

/// Collect every uppercase-tag [`Element`] entity under `root`, depth-first.
fn collect_uppercase_elements(
	world: &World,
	entity: Entity,
	out: &mut Vec<Entity>,
) {
	if let Some(element) = world.entity(entity).get::<Element>() {
		if element.tag().starts_with(|ch: char| ch.is_uppercase()) {
			out.push(entity);
		}
	}
	if let Some(children) = world.entity(entity).get::<Children>() {
		for child in children.iter() {
			collect_uppercase_elements(world, child, out);
		}
	}
}

/// Reconstruct a [`BsxElement`] AST from a parsed element entity (tag,
/// attributes, and value/element children), for MDX re-resolution.
fn reconstruct_element(world: &World, entity: Entity) -> Option<BsxElement> {
	let tag = world.entity(entity).get::<Element>()?.tag().to_string();
	let attributes = world
		...
		.map(|attrs| {
			attrs
				.iter()
				.filter_map(|attr| reconstruct_attribute(world, attr))
				.collect()
		})
		.unwrap_or_default();
	let children = world
		...
		.map(|kids| {
			kids.iter()
				.filter_map(|child| reconstruct_node(world, child))
				.collect()
		})
		.unwrap_or_default();
	Some(BsxElement {
...
/// Reconstruct an attribute (key plus literal-string value) from its entity.
fn reconstruct_attribute(world: &World, entity: Entity) -> Option<BsxAttribute> {
	let key = world.entity(entity).get::<Attribute>()?.to_string();
	let value = match world.entity(entity).get::<Value>() {
		Some(Value::Str(string)) => AttrValue::Str(string.to_string()),
		Some(_) | None => AttrValue::Flag,
	};
	Some(BsxAttribute { key, value })
}

/// Reconstruct a child node (element or text) from its entity.
fn reconstruct_node(world: &World, entity: Entity) -> Option<BsxNode> {
	if world.entity(entity).get::<Element>().is_some() {
		return reconstruct_element(world, entity).map(BsxNode::Element);
	}
	if let Some(Value::Str(text)) = world.entity(entity).get::<Value>() {
		return Some(BsxNode::Text(text.to_string()));
	}
	None
}
