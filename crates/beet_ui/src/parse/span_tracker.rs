...
/// # use beet_core::prelude::*;
/// let input = "hello\nworld\nfoo";
/// let lookup = SpanLookup::new(input, WsPath::new("test.txt"));
...
impl SpanLookup {
	...
	pub fn new(input: &str, path: WsPath) -> Self {
...
/// # use beet_ui::prelude::*;
/// # use beet_core::prelude::*;
/// let mut tracker = SpanTracker::new(WsPath::new("foo.txt"));
...
mod test {
	...
	fn lookup_span_of_slice() {
		...
		span.path().xpect_eq(WsPath::new("test.txt"));
	}
	...
	fn empty_text_stays_at_default() {
		let mut tracker = SpanTracker::new(WsPath::new("test.txt"));
		tracker.extend("");
	...
	fn into_full_span() {
		let tracker = {
			let mut t = SpanTracker::new(WsPath::new("test.txt"));
			t.extend("line1\nline2\n");
			t
		};
