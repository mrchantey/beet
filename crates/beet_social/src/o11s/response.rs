...
//!
//! This module contains the [`Body`] (aliased as `ResponseBody`) and all related
//! types returned from the `/v1/responses` endpoint.
//!
//! # Response Structure
//!
//! A response contains:
//! - **Status**: The lifecycle state (`in_progress`, `completed`, `failed`, `incomplete`)
//! - **Output**: Array of items generated by the model (messages, function calls, reasoning)
//! - **Usage**: Token consumption statistics
//! - **Metadata**: Request parameters echoed back for reference
//!
//! # Response Status Lifecycle
//!
//! Responses follow a state machine:
//!
//! ```text
...
//! ```
//!
//! - `completed`: Model finished successfully
//! - `failed`: An error occurred (check the `error` field)
//! - `incomplete`: Model was interrupted (check `incomplete_details` for reason)
//!
//! # Output Items
//!
//! The `output` array contains items the model generated:
//!
//! - **Message**: Text output with optional annotations (citations)
//! - **FunctionCall**: Tool invocation with name and JSON arguments
//! - **Reasoning**: Model's reasoning trace (for reasoning models)
//!
//! # Extracting Text
//!
...
//!     println!("Call {} with args: {}", fc.name, fc.arguments);
//! }
//! # }
//! ```
//!
...
pub struct Body {
	...
	/// Optional for provider compatibility (ollama currently omits this in streaming).
	#[serde(default)]
	...
	// ollama will output: null on final message
