...
/// [`DisableBootOnLoad`]), then booted explicitly through its boot slot — the
/// exchange->boot bridge ([`ExchangeToBoot`]) with a dynamically-loaded target — so
/// the workspace entry serves only when `serve` is invoked, never on an always-on
/// `BootOnLoad`. The boot call parks on the servers' `Running` keep-alive, so this
...
pub async fn Serve(cx: ActionContext<Request>) -> Result<Response> {
	...
	let root = build_site(&caller, site_dir, entry).await?;
	caller
		.world()
		.entity(root)
