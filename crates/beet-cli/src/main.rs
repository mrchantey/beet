...
//! `load`/`clear`/`reset`/`dump`/`call`. It carries *retained state* — on startup
...
//! (`run`, `run-wasm`, `export-pdf`, …) ship as a loadable `default-cli` scene
...
fn main() -> AppExit {
	...
	App::new()
		.add_plugins((
			...
			// the style rule set `beet run` sites render with
...
fn spawn_host(world: &mut World) {
	world.spawn((CliServer, default_router(), children![
		SceneLoad, SceneClear, SceneReset, SceneDump, SceneCall,
	]));
}
