...
Every script is an *arrow function body*, so `await` is legal anywhere and the body answers the way an arrow does: a body whose trimmed source starts with `{` is a block and answers with `return`, any other body is an expression and answers with its own value. Being JavaScript's own rule, it comes with JavaScript's own workaround, an object literal is parenthesised: `({ name: "ada" })`. What the body answers is deserialized into `Output`; a `()` output discards it, and a typed output that never arrives is an error naming the body's shape.
