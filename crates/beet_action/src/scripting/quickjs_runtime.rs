...
/// output. JSON is the marshalling currency so neither side needs a bespoke
/// rquickjs serde bridge. QuickJS errors are flattened to a message.
...
mod test {
	...
	async fn increments_a_number() {
		AsyncPlugin::world()
			.spawn(Script::<i64, i64>::quickjs("input + 1"))
			.call::<i64, i64>(41)
			.await
			.unwrap()
			.xpect_eq(42);
	...
	async fn concatenates_strings() {
		AsyncPlugin::world()
			.spawn(Script::<String, String>::quickjs(r#""hello " + input"#))
	...
	async fn mutates_a_struct_field() {
		AsyncPlugin::world()
			.spawn(Script::<Player, Player>::quickjs(
	...
	async fn parse_errors_propagate() {
		AsyncPlugin::world()
			.spawn(Script::<i64, i64>::quickjs("this is not valid js ((("))
			.call::<i64, i64>(0)
			.await
			.unwrap_err();
	}
