...
pub fn panic_expected_received_display<T1: Display, T2: Display>(
	...
	panic_ext::panic_with_expected_received_display(expected, received)
}
/// Must be called at [`SweetError::BACKTRACE_LEVEL_3`]
pub fn panic_expected_received_debug<T1: Debug, T2: Debug>(
	...
	panic_ext::panic_with_expected_received_debug(expected, received)
}
...
pub fn assert_diff(expected: impl AsRef<str>, received: impl AsRef<str>) {
	...
	if expected != received {
		panic_ext::panic_with_str(&crate::utils::pretty_diff::inline_diff(
			expected, received,
		));
	}
}
...
{
	if received.is_none() {
		panic_ext::panic_with_expected_received_display(&"Some", &received);
	}
}
...
{
	if received.is_some() {
		panic_ext::panic_with_expected_received_display(&"None", &received);
	}
}
