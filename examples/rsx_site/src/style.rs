...
pub mod classes {
	...
	/// Site-local class for the showcase rows that lay widget variants out side
	/// by side.
	pub const DESIGN_ROW: ClassName = ClassName::new_static("design-row");
}

/// A horizontal flex row with a gap, for the showcase page that lays out widget
/// variants side by side, styling [`classes::DESIGN_ROW`]. The typed twin of a
/// markup `<Rule>`.
///
