...
/// Handles all SSH events for connected clients.
fn on_recv(
	ev: On<SshRecv>,
	...
	match ev.event().inner() {
		SshEvent::Connect => {
			// Send a welcome banner and insert the input buffer.
			...
			commands
				...
				.trigger_target(SshSend(SshEvent::text(greeting)));
		}
		SshEvent::Data(_) => {
			let Ok(mut buf) = buffers.get_mut(conn) else {
				return;
			};
			let Some(bytes) = ev.event().as_bytes() else {
				return;
			};
			// Clone bytes to avoid borrow issues with commands below.
				...
				match byte {
					...
					0x7f | 0x08 => {
						if buf.0.pop().is_some() {
							commands.entity(conn).trigger_target(SshSend(
								SshEvent::text("\x08 \x08"),
							));
						}
					}
