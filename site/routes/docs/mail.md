...
The box logs to two files, and the agent tails both into one CloudWatch group: `stalwart.log` is the unit's stdout (bootstrap mode, the backup scripts, a panic) and the daily-rotated `stalwart.<date>` is the claimed server's own tracer, declared without ANSI colour so the timestamp leads every line and the agent dates events at write rather than at ingestion. The agent follows only the newest file matching a glob, which is exactly right for a rotated series; our first week shipped nothing but bootstrap lines because only the first file was tailed.
...
**Hand step.** Enable DNSSEC on the zone (`PATCH /zones/<id>/dnssec {"status":"active"}` on the Cloudflare API). If Cloudflare is also your registrar the DS record publishes to the parent automatically, but not quickly: ours sat `pending` for just over a fortnight (enabled 2026-08-28, at the `.com` parent by 2026-09-15) with nothing to do but re-query, and the support ticket we had drafted in our heads was never needed. Verify with a DoH query for `type=DS` at the apex, and an apex MX query checking the `AD` flag; `GET /zones/<id>/dnssec` reports `active` once the parent has it. The zone keeps resolving unsigned-style until the DS lands, so mail is unaffected either way, and until it lands there is no DANE to be had.

**Hand step, and a judgement call.** MTA-STS ships in `testing` mode. Flip it to `enforce` after two clean weeks of TLS-RPT reports and not before. A `testing` policy withholds no mail whether or not it is fetched; an `enforce` policy withholds mail from senders whose TLS your box fails to satisfy, which is the point of it and also the risk. Expect the report stream to be EMPTY rather than clean: a reporter (Google and Microsoft, in practice) writes only for days on which it delivered to you, so a domain nobody has mailed yet produces no reports at all, and two weeks of silence is not two weeks of evidence. When that is where you are, check directly what a report would have checked for you: the policy's `mx:` equals your MX record; port 25 offers STARTTLS with a certificate for that exact name from a publicly trusted CA (`openssl s_client -starttls smtp -verify_hostname <mx> -verify_return_error`, run on the box against `127.0.0.1:25` with `-servername`, since neither a residential ISP nor EC2's default egress block will let you reach 25 from outside); the policy fetches over valid TLS with no redirect; and an outside vantage on 25 agrees, for which [internet.nl](https://internet.nl/mail/) is free and checks trust chain and name match. Then flip, knowing the shape of being wrong: an `enforce` failure is a deferral at the sender, not a bounce, and the way back is the one attribute and a deploy, which bumps the `_mta-sts` id and has senders re-fetch. One message from a Gmail account to your domain will provoke a Google report the next day, which is the cheapest way to have `enforce` judged after the fact. Also note that a wrangler custom domain's certificate is not instantly live at every edge: our deploy verified both policy fetches, a `curl` two minutes later got a TLS error from the same machine, and five minutes after that it was clean. Do not roll back over that.
...
6. **Everything else regenerates:** config from git, the box from user data, secrets re-mintable. The exceptions are the DKIM private key once its selector is published and any relay credential a human minted by hand, so every deploy and provision ends by exporting the stack's whole secret store into a secrets document, twice: the committed `infra/secrets/<app>--<stage>.toml` in the repo (`<SecretsExport document={$mail_secrets}/>`, overwritten only when a secret changed, so git holds the history) and a dated series in the cold bucket (`<SecretsExport document={$mail_cold} dated=true/>`). Both are sealed with age to the humans listed in the entry's own `secrets.toml`, one identity per person, each living with its human and in neither cloud; `beet secrets/restore` is the inverse, and the [secrets tutorial](/docs/secrets) is the whole story.
...
### Who mints the cold copy's credential
...
The one credential the stack does not mint is the age identity the exports are sealed to, because its whole point is to be held by a human off both clouds: one per person, the public halves listed in the entry document's `default` group, which every export copies. Making one, backing it up, adding and removing a person, restoring an export into a fresh account and reading one with the `age` command line alone are the [secrets tutorial](/docs/secrets).

Then the deploy tail runs three steps after the mail probe, every time: the two `<SecretsExport/>`s, `<MailColdPush/>` (which starts the box's nightly unit right now over ssh and waits for it, so the path is exercised before the timer ever fires) and `<MailColdProbe/>`, which reads the newest snapshot back from the cold side and the live side and compares digests, does the same for one blob, checks the snapshot was *taken* within 36 hours by the clock in its own key (its last-modified in the cold bucket would only say when it was copied), and opens the newest export with your identity to prove it is this stack's and readable. `mail/cold-probe` is the same check on any other day.
...
The drill is a whole parallel stage, not a spare database file. `deploy` it, `restore-drill` into it, `destroy` it. A non-production Stalwart volume is disposable by default precisely so this works: the drill tears down the same declaration production protects, without either stage knowing about the other. The destroy is whole: after `tofu destroy` has taken what terraform owns, `<StackTeardown/>` sweeps the stage's parameter store prefix, so the admin password, the DKIM key and the mailbox credentials the actions minted go with the stack and nothing is left to delete by hand. That sweep is unconditional, on production too, which is why the secrets exports at the end of every deploy exist: a destroyed stack's secrets live on in the committed repo document and the cold bucket's dated series, and nowhere else.
...
- Keeping the age identity somewhere that is neither cloud. An export nobody can decrypt is a very safe way to lose the DKIM key. `beet vault/backup --qr` writes a passphrase-encrypted copy for a stick and prints one for paper; two dumb places are enough, and a password manager is for your console logins, not for this.
