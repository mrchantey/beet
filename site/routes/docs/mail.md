...
- Keeping the `age` identity somewhere that is neither cloud. An export nobody can decrypt is a very safe way to lose the DKIM key. A passphrase-encrypted copy (`age -p`) in two dumb places is enough; a password manager is for your console logins, not for this.
