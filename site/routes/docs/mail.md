...
6. **Everything else regenerates:** config from git, the box from user data, secrets re-mintable. The exceptions are the DKIM private key once its selector is published and any relay credential a human minted by hand, so every deploy and provision ends by exporting the whole parameter prefix into the cold bucket, `age`-encrypted to the recipients declared in the entry (`<MailSecretsExport recipients={["age1.."]}/>`, one per human who may restore). Each identity lives with its human, backed up with `age -p` or on paper, and in neither cloud.
...
The one credential the stack does not mint is the age identity the export is encrypted to, because its whole point is to be held by a human off both clouds. Generate one per person with `age-keygen -o <somewhere private>`; the public halves go in the entry as `recipients`, any one identity decrypts, and no identity is shared. Decrypting an export is `age -d -i <identity> <file>`, and the document inside is every parameter's name, type and value plus a one-line restore instruction.
...
The drill is a whole parallel stage, not a spare database file. `deploy` it, `restore-drill` into it, `destroy` it. A non-production Stalwart volume is disposable by default precisely so this works: the drill tears down the same declaration production protects, without either stage knowing about the other. The destroy is whole: after `tofu destroy` has taken what terraform owns, `<StackTeardown/>` sweeps the stage's parameter store prefix, so the admin password, the DKIM key and the mailbox credentials the actions minted go with the stack and nothing is left to delete by hand. That sweep is unconditional, on production too, which is why the secrets export at the end of every deploy exists: a destroyed stack's secrets live on in the cold bucket and nowhere else.
