...
mod test {
	...
	fn insert_works() {
		...
		let insert_lib = insert("crates_sweet/sweet-cli/src/foo.rs").unwrap();
	...
	fn remove_works() {
		...
		let remove_lib = remove("crates_sweet/sweet-cli/src/automod").unwrap();
		...
		let remove_mod =
			remove("crates_sweet/sweet-cli/src/bench/bench_assert.rs").unwrap();
		expect(&remove_mod)
			.not()
